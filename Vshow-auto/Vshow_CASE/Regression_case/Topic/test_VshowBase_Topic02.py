...
@allure.feature("动态首页点击-更多 查看话题列表所有话题--进入话题列表页")
class Test_VSHOWBASE_Topic02:
    ...
    def test_produce(self):
        ...
        wait_for_all_elements(new_driver, [(AppiumBy.ID, self.driver_data_2.get("appPackage")+":id/tvTitle"),
                                           (AppiumBy.ID, self.driver_data_2.get("appPackage") + ":id/tvTitle")
                                           ], step_name="检查当前页面是否出现话题title的ID")
        wait_for_page_text(new_driver,["参加","参加"],step_name="检查当前页面是否有2个参加")
...
if __name__ == "__main__":
    pytest.main(["-s", "VSHOWBASE_Topic02"])
