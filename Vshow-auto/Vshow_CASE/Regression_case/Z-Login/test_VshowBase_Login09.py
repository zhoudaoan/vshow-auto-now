...
from Vshow_TOOLS.common_actions import click_button_by_text, click_element_by_id, wait_for_page_text
...
@allure.feature("登录成功，退出，在登录")
class Test_VSHOWBASE_Login09:
    """
    1、类名只需要更改后面的四位数字
    """
    ...
    def teardown_method(self):
        ...
        重新登录账号，并清除手机后台所有应用
        """
        new_driver = more_driver(self.driver_data, self.appium_url)
        register(new_driver, self.driver_data.get("appPackage"))
        new_login(new_driver, self.tel, self.pwd, self.driver_data.get("appPackage"),tag=2)
        new_driver.quit()
        clear_app_background(self.driver_data.get("udid"),self.driver_data.get("appPackage"))


if __name__ == "__main__":
    pytest.main(["-s", "VSHOWBASE_Login09.py"])
