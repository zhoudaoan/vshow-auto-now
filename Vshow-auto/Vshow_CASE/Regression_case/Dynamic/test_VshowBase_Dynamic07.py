...
@allure.feature("点击进入动态详情页--动态详情页展示正常")
class Test_VSHOWBASE_Dynamic07:
    ...
    def setup_method(self):
        logger.info("主播端--登录app，进入动态")
        self.new_driver = more_driver(self.driver_data_2, self.appium_url_2)
        self.userID_newDriver, self.userNameNewDriver = get_user_id(self.new_driver)
    ...
    def test_produce(self, driver):
        ...
        scroll_to_element(
            driver,
            locator=(AppiumBy.ID, self.driver_data.get("appPackage") + ":id/clTranslate"),
            direction="up",
            max_swipes=10
        )
        wait_for_page_text(driver,["动态详情",self.userNameNewDriver], step_name="进入动态详情", match_all=True)

        logger.info("主播端--查看动态详情")
        search_user(self.new_driver, self.userID_newDriver)
        click_element_by_id(self.new_driver, element_id=self.driver_data.get("appPackage") + ":id/ivAvatar",
                            step_name="点击主播头像进入详情页")
        ...
        wait_for_page_text(self.new_driver, ["动态详情", self.userNameNewDriver], step_name="进入动态详情",
                           match_all=True)
