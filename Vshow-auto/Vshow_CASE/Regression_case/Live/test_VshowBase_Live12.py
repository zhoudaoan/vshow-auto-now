...
@allure.feature("直播间上麦--用户申请上麦主播可以选择接受或拒绝")
class Test_VSHOWBASE_Live12:
    ...
    def test_produce(self, driver):
        ...
        logger.info("主播端点击拒绝按钮")
        click_element_by_id(self.new_driver, element_id=self.driver_data_2.get("appPackage")+":id/iv_up_mic", step_name="点击直播间上麦按钮")
        click_element_by_id(self.new_driver, element_id=self.driver_data_2.get("appPackage")+":id/ivRefuse", step_name="点击拒绝上麦按钮")
        logger.info("观众端弹出弹窗")
        wait_for_page_text(driver, ["您已被拒绝"])
        click_element_by_id(driver, element_id=self.driver_data.get("appPackage")+":id/positiveButton", step_name="拒绝上麦后点击OK按钮")
...
if __name__ == "__main__":
    pytest.main(["-s", "VSHOWBASE_Live12.py"])
