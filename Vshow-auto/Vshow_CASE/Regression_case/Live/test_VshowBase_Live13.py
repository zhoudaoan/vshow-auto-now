...
from Vshow_Page.Vshow_H5.vshow_task_h5 import get_user_id, live_room, search_user, close_live_or_party_room
...
@allure.feature("直播间上麦--主播和麦上用户切后台是否崩溃")
class Test_VSHOWBASE_Live13:
    ...
    def test_produce(self, driver):
        ...
        click_element_by_id(driver, element_id=self.driver_data.get("appPackage")+":id/iv_up_mic", step_name="点击直播间上麦按钮")
        wait_for_page_text(driver,["您已成功排麦，等待主播同意"])
        click_element_by_id(driver, element_id=self.driver_data.get("appPackage")+":id/positiveButton", step_name="点击排麦成功的OK按钮")
        ...
        logger.info("观众端切换APP到后台等待10秒钟后切回前台")
        driver.background_app(10)
        wait_for_page_text(driver,["所有","房间","聊天"])
        logger.info("主播端切换APP到后台等待10秒钟后切回前台")
        self.new_driver.background_app(10)
        wait_for_page_text(self.new_driver,["所有","房间","聊天"])
        logger.info("主播端和观众端正常可以退出直播间")
        click_element_by_id(driver, element_id=self.driver_data.get("appPackage")+":id/liveClose", step_name="点击直播间的关闭按钮，关闭观众端直播间")
...
if __name__ == "__main__":
    pytest.main(["-s", "VSHOWBASE_Live13.py"])
